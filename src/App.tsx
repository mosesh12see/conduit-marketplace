...
function makeToast(message: string, variant: Toast['variant'] = 'default'): Toast {
  return { id: `toast-${++toastCounter}`, message, variant };
}
...
export default function App() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const newListing = makeListing(0);
      newListing.fresh = true;
      setFeed(prev => [newListing, ...prev].slice(0, 50));
      // Clear fresh flag after 4s
      setTimeout(() => {
        setFeed(prev => prev.map(l => l.id === newListing.id ? { ...l, fresh: false } : l));
      }, 4000);
    }, tweaks.feedSpeed * 1000);
    return () => clearInterval(interval);
  }, [tweaks.feedSpeed]);

  // TICK: increment age, decrement expiresIn every 1s
  useEffect(() => {
    const tick = setInterval(() => {
      setFeed(prev => prev.map(l => ({
        ...l,
        age: l.age + 1,
        expiresIn: Math.max(0, l.expiresIn - 1),
      })));
      setActiveBids(prev => prev.map(b => ({
        ...b,
        expiresIn: Math.max(0, b.expiresIn - 1),
      })));
    }, 1000);
    return () => clearInterval(tick);
  }, []);

  // OUTBID SIMULATION: every 18s, 30% chance to outbid a leading bid
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setActiveBids(prev => {
        ...
        return prev.map(b =>
          b.id === target.id
            ? { ...b, status: 'outbid' as const, topBid: b.yourBid + bump, expiresIn: b.expiresIn }
            : b
        );
      });
    }, 18000);
  ...
  // FILTERED FEED
  const filteredFeed = activeVertical === 'all'
    ? feed
    : feed.filter(l => l.vertical === activeVertical);

  // VERTICAL COUNTS
  const verticalCounts: Record<string, number> = { all: feed.length };
  for (const v of VERTICALS) {
    verticalCounts[v] = feed.filter(l => l.vertical === v).length;
  }
    ...
    <div className="app-shell">
      ...
      <main className="main">
        <Tabs
          ...
          tabs={[
            { id: 'live', label: 'Live feed', count: filteredFeed.length },
          ...
          <>
            {tweaks.showStats && (
              <div className="stats-strip">
                ...
                <StatCard label="Leads acquired" value={String(INITIAL_STATS.leadsAcquired)} delta={`+${INITIAL_STATS.leadsLastHour} last hour`} positive />
                <StatCard label="Avg CPL" value={`$${INITIAL_STATS.avgCpl.toFixed(2)}`} delta={`${INITIAL_STATS.cplDelta}% vs 7d`} positive={INITIAL_STATS.cplDelta < 0} />
            ...
            <div className="feed-header">
              <h2 className="feed-title">
                {activeVertical === 'all' ? 'All verticals' : VERTICAL_LABELS[activeVertical as Vertical]}
              </h2>
              ...
              <div className="feed-filters">
                <button className="filter-chip">Platinum only</button>
                <button className="filter-chip">Top score</button>
                <button className="filter-chip"><Icon name="filter" size={14} /> Filters</button>
              </div>
            </div>
            <div className="feed-list" ref={feedRef}>
              {filteredFeed.map(listing => (
                <ListingCard
                  key={listing.id}
                  listing={listing}
                  onBid={setModalListing}
                  isWon={wonIds.has(listing.id)}
                />
              ))}
            </div>
          </>
        ...
        {activeTab === 'deals' && (
          <table className="deals-table">
            ...
            <tbody>
              {activeBids.map(bid => (
                <tr key={bid.id}>
                  <td><span className={`listing-tier ${bid.tier}`}>{bid.tier}</span></td>
